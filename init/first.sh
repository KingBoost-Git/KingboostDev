...
if ! [ "$(ls -A $HOME/kingboostdev/www/)" ]; then
  echo '~~~~~ ЗАПУСК ОКРУЖЕНИЯ ~~~~~'
  cd $HOME/kingboostdev && docker compose up -d --build && cd $HOME
  echo '~~~~~ ПОДГРУЗКА РЕПОЗИТОРИЯ ~~~~~'
  cd $HOME/kingboostdev/www/ && git clone git@github.com:KingBoost-Git/kingboost_dtv.git .
  echo '~~~~~ КОПИРОВАНИЕ КОНФИГОВ ~~~~~'
  cp -rf $HOME/kingboostdev/init/www/ $HOME/kingboostdev/www/
